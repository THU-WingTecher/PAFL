...
EXP_ST u64* global_hit_bits;
static u64 hit_bits[MAP_SIZE];        /* @RB@ Hits to every basic block transition */

static u64 is_min_branch[MAP_SIZE];   /*@Ljie@ if a branch is a minimum count branch of a seed*/ 
...
struct queue_entry {
  ...
  u32 bitmap_size,                    /* Number of bits set in bitmap     */
      ...
      fuzzed_times;                   /*The number of times the seed was fuzzed*/
  ...
  //u64 min_branch_hit;                  /* @Ljie@ the minist hit branch count*/
  u64 min_branch_id;                  /* @Ljie@ the minist hit branch id*/
...
static u32 vanilla_afl = 1000;      /* @RB@ How many executions to conduct in vanilla AFL mode*/
static u32 update_min_branch = 1000000;
static u32 has_basic_statistics = 0;      /* @Ljie@ if exe fair select */
static u32 fair_rate = 100;

static bool add_select = false; /*Add balanced seed selection*/
static bool add_reserved_mutation = false; /*Add reserved mutation*/
static bool skip_when_null = false;  /*Skip the seeds when rare branch set is null, just for fairfuzz mode*/
static int rare_rank = 0;   /*For hierarchical fuzzing*/
static int instance_num = 1; /*Total number of fuzzer instance*/
static int instance_index = 0; /*The index of this fuzzer instance*/
// static int any_optimizations=0; /*Whether add any optimizations*/
static int task_division=0; /*Whether use fuzzing task division*/
...
//static float sum_reciprocal_hit_count=0;    /*@Ljie@ the sum of the reciprocal of each minimal hit count the covered branch*/
...
void mylog (char const *fmt, ...) { 
    static FILE *f = NULL;
    if (f == NULL) {
      u8 * fn = alloc_printf("%s/hit_sync.log", out_dir);
      f= fopen(fn, "w");
      ck_free(fn);
    }
    va_list ap;
...
static u32 * is_rb_hit_mini_range(u8* trace_bits_mini, int rank){
  int * rarest_branches = get_lowest_hit_branch_ids();
  u32 * branch_ids = ck_alloc(sizeof(u32) * MAX_RARE_BRANCHES);
  u32 * branch_cts = ck_alloc(sizeof(u32) * MAX_RARE_BRANCHES);
  int min_hit_index = 0;
  
  int start = rank* MAP_SIZE/instance_num;
  int end = start + MAP_SIZE/instance_num;
...
/*  @Ljie@ 
    find the minimum hit count of the branch the seed covered
*/
// static u64 find_minimum_hit_branch_count(){
//   u64 minimum_hit = ULONG_MAX;
//   for (int i = 0; i < MAP_SIZE; i++){
//     if ((trace_bits[i] > 0) && (hit_bits[i] < minimum_hit))
//       minimum_hit = hit_bits[i];
//   }
//   return minimum_hit;
// }
// return true if the min branch is still min branch
// static bool update_minimum_hit_branch_id(struct queue_entry * uqueue){
//   u64 minimum_hit = hit_bits[uqueue->min_branch_id];
//   bool still_min = true;
//   for (int i = 0; i < MAP_SIZE; i++){
//     if (unlikely (uqueue->trace_mini[i >> 3]  & (1 <<(i & 7)) )) {
//       if(hit_bits[i] < minimum_hit) {
//         minimum_hit = hit_bits[i];
//         uqueue->min_branch_id = i;
//         still_min = false;
//       }
//     }
//   }
...
static void update_minimum_entry_hit_branch_id(struct queue_entry * uqueue) {
  u64 minimum_hit = ULONG_MAX;
  u64 minimum_id = -1;
  for (int i = 0; i < MAP_SIZE; i++){
    if (unlikely (uqueue->trace_mini[i >> 3]  & (1 <<(i & 7)) )) {
  ...
  if(minimum_id != -1) {
    is_min_branch[minimum_id]=1;
  }
  
}

static void update_minimum_hit_branch() {
  struct queue_entry * mqueue = queue;
  memset(is_min_branch, 0, sizeof(is_min_branch));
  while(mqueue) {
    update_minimum_entry_hit_branch_id(mqueue);
    mqueue = mqueue->next;
  }
    
}
/*  @Ljie@ 
    find the id of minimum hit count of the branch the seed can cover
*/
...
static void increment_hit_bits(){
  for (int i = 0; i < MAP_SIZE; i++){
    if ((trace_bits[i] > 0) && (hit_bits[i] < ULONG_MAX))
      hit_bits[i]++;
      // if(0 && is_min_branch[i] == 1) {
      //     struct queue_entry * mqueue = queue;
      //     bool still_min = false;
      //     while(mqueue) {
      //       if(mqueue->min_branch_id == i) {
      //         still_min = still_min || (update_minimum_hit_branch_id(mqueue));
      //       }
      //       mqueue = mqueue->next;
      //     }
      //     if(!still_min) {
...
static u64 find_minimum_hit_branch_id() {
  if(!add_select && schedule != FAST && task_division != 1) {
    return -1;
  }
  u64 minimum_hit = ULONG_MAX;
  u64 minimum_id = -1;
  for (int i = 0; i < MAP_SIZE; i++){
    if ((trace_bits[i] > 0) && (hit_bits[i] < minimum_hit)) {
        //ACTF("trace_bits is %d",trace_bits[i]);
        if(hit_bits[i] == 0) {
          increment_hit_bits();
        }
        minimum_hit = hit_bits[i];
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  //q->min_branch_hit = find_minimum_hit_branch_count();
  
  q->min_branch_id =  find_minimum_hit_branch_id();
  is_min_branch[q->min_branch_id] = 1;
  

  if(0 && not_on_tty) {
     ACTF("Min branch id is %llu",
          q->min_branch_id);
    fflush(stdout);
  }
...
static void remove_hit_bits(void) {

  shmctl(hit_shm_id, IPC_RMID, NULL);
  del_semvalue();
...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  //u64 hit_p2 = next_p264(hit_bits[q->min_branch_id]);
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
       ...
       if (top_rated[i]) {
         ...
         //u32 top_rated_hit_p2 = next_p264(hit_bits[top_rated[i]->min_branch_id]);
         //if(hit_p2 > top_rated_hit_p2) continue;
...
void get_a_in_file(char * pathname) {
    DIR *dp;
    struct dirent *dirp;
    ...
    strcpy(pathname, in_dir);
    strcat(pathname, "/");
    while((dirp = readdir(dp)) != NULL) {
      if(dirp->d_type == 8){
        strcat(pathname, dirp->d_name);
      //#PFATAL("filename is %s\n",pathname);
        break;
      }
...
EXP_ST void setup_shm(char ** argv) {
  ...
  char pathname[256];
  get_a_in_file(pathname);

  //PFATAL("filename is %s",pathname);
  ...
  key_t key_master = ftok(pathname, 0);
 // if(is_master) {
   // hit_shm_id = shmget(key_master, MAP_SIZE*sizeof(u64), IPC_CREAT | IPC_EXCL | 0666);
   // sem_id = semget((key_t)key_master, 1, IPC_CREAT | IPC_EXCL | 0666);
  //}
 // else {
    hit_shm_id = shmget(key_master, MAP_SIZE*sizeof(u64), IPC_CREAT | 0666);
    sem_id = semget((key_t)key_master, 1, IPC_CREAT | 0666);
  //}
  
  if (hit_shm_id < 0) PFATAL("hit_cnt shmget() failed");

  if(!set_semvalue())  {  
      PFATAL("Failed to initialize semaphore\n");
  }
  global_hit_bits = shmat(hit_shm_id, NULL, 0);
  if(is_master) {
    memset(global_hit_bits, 0, MAP_SIZE);
  }
  atexit(remove_hit_bits);
}
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    q->min_branch_id = find_minimum_hit_branch_id();
      //ACTF("test q min_branch_id is %llu",q->min_branch_id);
    is_min_branch[q->min_branch_id] = 1;
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  if (prev_qp == queued_paths && prev_pf == pending_favored && 
      ...
      prev_uh == unique_hangs && prev_md == max_depth && prev_t_bits == t_bits) return;
  ...
  /* Fields in the file:
     ...
     unix_time, ,t_bits,cycles_done, cur_path, paths_total, paths_not_fuzzed,
     favored_not_fuzzed, unique_crashes, unique_hangs, max_depth,
     execs_per_sec */

  fprintf(plot_file, 
          "%llu, %u, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f\n",
          get_cur_time() / 1000, t_bits,queue_cycle - 1, current_entry, queued_paths,
...
//             "zzzz..." : "slow!");
//     // for(int i = 0; i < MAP_SIZE; i++) {
//     //   DEBUG_SYNC("In branch %d, hit_bits is %d\n", i, hit_bits[i]);
//     // }
...
static void show_stats(void) {
  ...
  banner_len = (crash_mode ? 24 : 22) + strlen(VERSION) + strlen(use_banner);
  banner_pad = (78 - banner_len) / 2;
  memset(tmp, ' ', banner_pad);

  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
          " (%s)",  crash_mode ? cPIN "peruvian were-rabbit" : 
          cYEL "PAFL", use_banner);
...
#define SP40    SP20 SP20
#define SP50    SP40 SP10
#define SP55    SP50 SP5
#define SP60    SP20 SP40
#define SP70    SP60 SP10

  /* Lord, forgive me this. */

  SAYF(SET_G1 bSTG bLT bH bSTOP cCYA " process timing " bSTG bH20 bH10 bH bHB
       bH bSTOP cCYA " overall results " bSTG bH bRT "\n");
  ...
  SAYF(bV bSTOP "        run time : " cRST "%-28s " bSTG bV bSTOP
  ...
  if (!dumb_mode && (last_path_time || resuming_fuzz || queue_cycle == 1 ||
    ...
    SAYF(bV bSTOP "   last new path : " cRST "%-28s ",
         DTD(cur_ms, last_path_time));
  ...
  SAYF(bV bSTOP " last uniq crash : " cRST "%-28s " bSTG bV bSTOP
       "uniq crashes : %s%-2s  " bSTG bV "\n",
       DTD(cur_ms, last_crash_time), unique_crashes ? cLRD : cRST,
       tmp);
#if 0
  sprintf(tmp, "%s%s", DI(unique_hangs),
         (unique_hangs >= KEEP_UNIQUE_HANG) ? "+" : "");
  SAYF(bV bSTOP "  last uniq hang : " cRST "%-20s " bSTG bV bSTOP 
       "   uniq hangs : " cRST "%-6s " bSTG bV "\n",
       DTD(cur_ms, last_hang_time), tmp);
#endif


  SAYF(bVR bH bSTOP cCYA " cycle progress " bSTG bH15 bHB bH bSTOP cCYA
       " map coverage " bSTG bHT bH bH15 bH2 bH bVL "\n");
  ...
  SAYF("    map density : %s%-16s " bSTG bV "\n", t_byte_ratio > 70 ? cLRD : 
       ((t_bytes < 200 && !dumb_mode) ? cPIN : cRST), tmp);

  sprintf(tmp, "%s (%0.02f%%)", DI(cur_skipped_paths),
          ((double)cur_skipped_paths * 100) / queued_paths);

  SAYF(bV bSTOP " paths timed out : " cRST "%-12s " bSTG bV, tmp);

  sprintf(tmp, "%0.02f bits/tuple",
          t_bytes ? (((double)t_bits) / t_bytes) : 0);

  SAYF(bSTOP " count coverage : " cRST "%-16s " bSTG bV "\n", tmp);

  SAYF(bVR bH bSTOP cCYA " stage progress " bSTG bH15 bX bH bSTOP cCYA
       " findings in depth " bSTG bH15 bVL "\n");
  ...
  SAYF(bV bSTOP " now trying : " cRST "%-17s " bSTG bV bSTOP 
       " favored paths : " cRST "%-17s " bSTG bV "\n", stage_name, tmp);
  ...
  SAYF(bV bSTOP " stage  : " cRST "%-21s " bSTG bV bSTOP, tmp);

  sprintf(tmp, "%s(%0.02f%%)", DI(queued_with_cov),
          ((double)queued_with_cov) * 100 / queued_paths);

  SAYF(" new edges on : " cRST "%-18s " bSTG bV "\n", tmp);
  ...
  if (crash_mode) {
    ...
    SAYF(bV bSTOP " total execs : " cRST "%-16s " bSTG bV bSTOP
         "   new crashes : %s%-17s " bSTG bV "\n", DI(total_execs),
         unique_crashes ? cLRD : cRST, tmp);

  } else {

    SAYF(bV bSTOP " total execs : " cRST "%-16s " bSTG bV bSTOP
         " total crashes : %s%-17s " bSTG bV "\n", DI(total_execs),
         unique_crashes ? cLRD : cRST, tmp);
  ...
  if (avg_exec < 100) {
    ...
    SAYF(bV bSTOP " exec speed : " cLRD "%-17s ", tmp);

  } else {

    sprintf(tmp, "%s/sec", DF(avg_exec));
    SAYF(bV bSTOP " exec speed : " cRST "%-17s ", tmp);
  ...
  SAYF (bSTG bV bSTOP " total tmouts : " cRST "%-18s " bSTG bV "\n", tmp);
#if 0
  /* Aaaalmost there... hold on! */
/*  SAYF(bVR bH cCYA bSTOP " fuzzing strategy yields " bSTG bH10 bH bHT bH10
       bH5 bHB bH bSTOP cCYA " path geometry " bSTG bH5 bH2 bH bVL "\n");
...
*/
...
#endif
  SAYF(bLB bH30 bH2 bHT bH30 bH5 bRB bSTOP cRST RESET_G1);
#if 0 
  /* Provide some CPU utilization stats. */
  ...
  if (cpu_core_count) {
    ...
    if (cpu_aff >= 0) {
      ...
      SAYF(SP30 cGRA "[cpu%03u:%s%3u%%" cGRA "]\r" cRST, 
           MIN(cpu_aff, 999), cpu_color,
           MIN(cur_utilization, 999));

    } else {

      SAYF(SP30 cGRA "   [cpu:%s%3u%%" cGRA "]\r" cRST,
           cpu_color, MIN(cur_utilization, 999));
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  if(add_select || schedule == FAST) {
    if(update_min_branch) --update_min_branch;
    if(update_min_branch == 0) {
      update_minimum_hit_branch();
      update_min_branch = 1000000;    
    }
  }
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  u64 min_hit_cnt = hit_bits[q->min_branch_id];

  if(0 && not_on_tty) {
    ACTF("min_hit_cnt is %llu...",min_hit_cnt);
    fflush(stdout);
  }
  ...
  switch (schedule) {
    case EXPLORE: 
      break;
    case FAST:
      if (q->fuzzed_times < 16) {
         factor = ((u32) (1 << (q->fuzzed_times))) / min_hit_cnt;
      } else
        factor = MAX_FACTOR / next_p264(min_hit_cnt);
      break;
  }
...
static u8 fuzz_one(char** argv) {
 ...
 if (skip_deterministic){
  ...
  // if(not_on_tty) {
  //   ACTF("Fuzzing test case #%u (weight is %lf)...",
  //        current_entry,weight);
  //   fflush(stdout);
  // }
  
  //trace_bits_mini[i >> 3]  & (1 <<(i & 7))

  //fuzzing task division
  if(task_division && has_basic_statistics) {
    int min_hit_id = queue_cur->min_branch_id;
    int start = instance_index* MAP_SIZE/instance_num;
    int end = start + MAP_SIZE/instance_num;
    

    if(min_hit_id < start || min_hit_id >= end) {
        // ACTF("skip this seed");
        // ACTF("start is %d, end is %d, min_hit_id is %d", start, end, min_hit_id);
        // fflush(stdout);
        return 1;
    }
    if(not_on_tty) {
  ...
  if(add_select && has_basic_statistics) {
    float sum_reciprocal_hit_count = 0;
    for (int i = 0; i < MAP_SIZE; i++){
      if(is_min_branch[i]) {
        sum_reciprocal_hit_count += 1/(float)hit_bits[i];
      }
    }
    float minimum_branch_recip = 1/(float)hit_bits[queue_cur->min_branch_id];
    float weight = minimum_branch_recip/sum_reciprocal_hit_count;
    float skip_rate = 1-weight;
    if(UR(100000) < skip_rate*100000*0.05) {
      // if(not_on_tty) {
      //   ACTF("skip test case #%u...",
      //      current_entry);
      //   fflush(stdout);
      // }
      return 1;
  ...
  u32 * min_branch_hits;
  if(not_on_tty) {
    ACTF("vanilla_afl is %d", vanilla_afl);
  }
  
  if(vanilla_afl == 0) {
    has_basic_statistics = 1;
  }
  
  if(!add_reserved_mutation) {
    vanilla_afl = 1;
    rb_fuzzing = 0;
  }
  else {
    min_branch_hits = is_rb_hit_mini_range(queue_cur->trace_mini,instance_index);
    if (min_branch_hits == NULL){
      if(UR(100) < fair_rate && !skip_when_null) {
        vanilla_afl = 1;
        rb_fuzzing = 0;
      }
    }
...
static void sync_hit_bits() {
    ...
    for(int i = 0; i < MAP_SIZE; i++) {
        ...
        if(!semaphore_p()) {
           ...
           PFATAL("Unable to wait signal");
        }
        //if(UR(100) < 25) {
          global_hit_bits[i] = get_max(hit_bits[i], global_hit_bits[i]);
          hit_bits[i] = global_hit_bits[i];
          //global_hit_bits[i] = tmp;
        //}
...
int main(int argc, char** argv) {
  ...
  u8  fair_rate_given = 0;
  u8  instance_num_given = 0;
  u8  rare_rank_given = 0;
  u8 instance_index_given=0;
  ...
  while ((opt = getopt(argc, argv, "+bq:rsi:o:f:m:t:T:dnCB:S:M:x:QFDPEp:N:R:I:")) > 0)
    ...
    switch (opt) {
      ...
      case 'F':/* add fair select*/
        if(add_select) FATAL("Multiple -F options not supported");
        add_select = true;
        break;

      case 'D': /* add reserved mutation*/
        if(add_reserved_mutation) FATAL("Multiple -D options not supported");
        add_reserved_mutation = true;
        break;

      case 'P': /* add fast power schedule*/
        if(schedule == FAST) FATAL("Multiple -P options not supported");
        schedule = FAST;
        break;
      case 'E': /* skip seeds when rare branch is null(for fairfuzz)*/
        if(skip_when_null) FATAL("Multiple -E options not supported");
        skip_when_null = true;
        break;
      case 'p': /*when rare branch is null, try AFL mode with fair_rate*/
        if(fair_rate_given) FATAL("Multiple -p options not supported");
        sscanf(optarg, "%u", &fair_rate);
        fair_rate = 1;
		fair_rate_given = 1;
        break;
      case 'N': /*the total number of fuzzer instances*/
        if(instance_num_given) FATAL("Multiple -N options not supported");
        sscanf(optarg, "%u", &instance_num);
		instance_num_given = 1;
        break;
      case 'h': /*the total number of fuzzer instances*/
        if(rare_rank_given) FATAL("Multiple -h options not supported");
        sscanf(optarg, "%u", &rare_rank);
		rare_rank_given = 1;
        break;
      case 'I': /*index of this instance, start from 0*/
        if(instance_index_given) FATAL("Multiple -I options not supported");
        sscanf(optarg, "%u", &instance_index);
        task_division = 1;
        int start = instance_index* MAP_SIZE/instance_num;
        int end = start + MAP_SIZE/instance_num;
        if (start >= MAP_SIZE || end > MAP_SIZE) {
            FATAL("The index of instance is our of range");
        }
		instance_index_given = 1;
        break;
      default:
